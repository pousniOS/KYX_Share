...
- (UILabel *)tfLabel
{
    if (_tfLabel==nil) {
        ...
        _tfLabel.frame = CGRectMake(0, self.subItemsss.frame.origin.y, KscreenWidth , 40);
...
- (UIColor *)setPwdJudgeAction:(UIColor *)color str:(NSString *)resultStr
{
    ...
    if (self.gestureModel == SetPwdModel) {
        ...
        if (isRight) {
            ...
            [KeychainData forgotPsw];
            // 失败
            self.tfLabel.text = @"密码验证失败请重新设置密码";//PSWFAILTSTRING;
            self.tfLabel.textColor = LABELWRONGCOLOR;
            [self shake:self.tfLabel];
...
- (UIColor *)validatePwdJudgeAction:(UIColor *)color str:(NSString *)resultStr
{
    if (self.gestureModel == ValidatePwdModel) {
        ...
        if (isValidate) {
            ...
            _validatePwdCount++;
            _result=NO;
            self.tfLabel.text = [NSString stringWithFormat:@"%@%lu次",PSWFAILTSTRING,_validatePwdCount];
            self.tfLabel.textColor = LABELWRONGCOLOR;
            [self shake:self.tfLabel];
            color = LABELWRONGCOLOR;
            [self blockAction:resultStr];
