...
//  PSOksales
//
//  Created by tangbin on 2017/2/14.
...
{
    return [self stretchableImageWithLeft:self.size.width * .5f top:self.size.height * .5f];
}

- (UIImage *)stretchableImageWithLeft:(CGFloat)left top:(CGFloat)top{
    return [self stretchableImageWithLeftCapWidth:left topCapHeight:top];
}
+ (UIImage *)stretchableImageToCenterWithName:(NSString *)name {
    return [self stretchableImageWithName:name left:0.5 top:0.5];
}
...
+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self _imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self _imageWithPDF:dataOrPath resize:YES size:size];
}
...
+ (UIImage *)_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        TBLog(@"UIImage+LFAdd error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        TBLog(@"UIImage+LFAdd error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        TBLog(@"UIImage+LFAdd error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasNewFunc) {
        ...
        if (err != kvImageNoError) {
            TBLog(@"UIImage+LFAdd error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
        err = vImageBuffer_Init(&scratch, effect.height, effect.width, format.bitsPerPixel, kvImageNoFlags);
        if (err != kvImageNoError) {
            TBLog(@"UIImage+LFAdd error: vImageBuffer_Init returned error code %zi for inputImage: %@", err, self);
            return nil;
        }
    } else {
